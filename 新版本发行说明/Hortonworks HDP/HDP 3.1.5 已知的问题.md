...
-	|	-	|	-	|	当调用SshActionExecutor的check（）方法时，Oozie将执行命令“ ssh <host-ip> ps -p <pid>”以确定SSH操作是否完成。但是，如果在操作状态检查期间与主机的连接失败，该命令将返回一个错误代码，但操作状态将确定为OK，这可能不正确。
-	|	-	|	-	|	相关错误消息：
