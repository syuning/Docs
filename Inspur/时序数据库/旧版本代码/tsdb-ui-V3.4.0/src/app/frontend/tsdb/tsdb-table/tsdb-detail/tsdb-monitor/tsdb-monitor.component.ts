...
export class TsdbMonitorComponent implements OnInit {
  ...
  generateEchartOption(legend: [string, string], dataX: any[], dataS1: any[], nameS1: string, dataS2: any[], nameS2: string) {
    const series = [];
    const series1 = { data: dataS1, type: 'line', name: nameS1 }; // '每分钟读取请求'
    const series2 = { data: dataS2, type: 'line', name: nameS2 }; // '每分钟写入请求'
  ...
  setEchartOption1(pto: any) {
    ...
    this.tsdbService.monitors(this.tsdbId, pto).subscribe(response => {
      ...
      console.log('图一！', response.message
      // , pto
      // , response
      // , this.echartOption1
      );
  ...
  setEchartOption2(pto: any) {
    ...
    console.log('图二！', response.message
    // , pto
    // , response
    // , this.echartOption2
    );
  ...
  setEchartOption3(pto: any): any {
    ...
    this.tsdbService.monitors(this.tsdbId, pto).subscribe(response => {
      ...
      if (this.tsdbService.validateResponseCode(response.code)) {
        ...
        this.echartOption3 = this.generateEchartOption(['每分钟写入数据点', ''], [], [], '每分钟写入数据点', [], '');
      }
      console.log('图三！', response.message
      // , pto
      // , response
      // , this.echartOption3
      );
