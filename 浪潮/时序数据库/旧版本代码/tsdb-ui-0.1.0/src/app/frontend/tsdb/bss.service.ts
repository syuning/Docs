...
// "bssFront": "http://bss.cloud.inspur.com"
  ...
  getUserQuota(userId: string, region: string, productLineCode: string, productTypeCode: string, quotaType: string): Observable<any> {
    return this.http.get<Config>(Environment.application.bssAPI + '/crm/quota?userId=' + userId
    + '&region=' + region + '&productLineCode=' + productLineCode + '&productTypeCode=' + productTypeCode + '&quotaType=' + quotaType
  ...
  userVerification(userId: string) {
    return this.http.get<Config>(Environment.application.serviceAPI + '/console/home/v1/usercertify?userId=' + userId
    , { observe: 'response' }).pipe(
  tap(response => response),
  catchError(this.handleError())
);
  ...
  generateTsdbOrderPto(userId: string, token: string, consoleOrderFlowId: string,
    ...
    instanceCount: number, dataPoint: number, diskVolume: number, instanceId: string) {
    ...
    const orderPto = {
      ...
      totalMoney: totalMoney, // 此处应为折后价actualMoney
      duration: duration, // 0
