...
由于这些默认图像可能不符合某些用户的要求（例如，当用户要求包括 *自定义操作系统强化custom OS hardening* ， *自定义库custom libraries* ， *自定义工具custom tooling* 等）时，Cloudbreak将允许您使用自己的 **自定义基本映像 custom base images** 。

要使用您自己的 *自定义基本映像(custom base images)* ，您必须：
...
|---|---|
| 镜像 images | Image `uuid`, same as the one that you specified in the `base-images` section. 图像`uuid`，与你在`base-images`部分中指定的相同。 |
| 版本 versions | The Cloudbreak version(s) for which you would like to use the images. 您要使用图像的Cloudbreak版本。 |
...
以下是一个示例图像目录JSON文件，其中包含两组自定义基本映像：
...
*  Azure，Google和OpenStack的自定义基本映像：
...
现在您已经创建了映像目录JSON文件，请将其注册到您的Cloudbreak实例。 您可以通过Cloudbreak UI，CLI或编辑Profile文件来完成此操作。
...
执行这些步骤后，映像目录将可用并自动选择为创建集群向导的映像目录下拉列表中的缺省条目。
...
要使用CLI注册自定义映像目录，请使用`cb imagecatalog create`命令。 请参阅[CLI文档]（cli-reference.md＃imagecatalog-create）。
...
4. 将映像目录文件保存在/ var / lib / cloudbreak-deployment / etc目录中的Cloudbreak计算机上。
...
6. 通过将export CB_IMAGE_CATALOG_URL添加到文件并将其设置为声明自定义映像的JSON文件的名称来编辑位于/ var / lib / cloudbreak-deployment中的Profile文件。 例如：`export CB_IMAGE_CATALOG_URL = custom-image-catalog.json`
...
8. 在**选择图像**下，选择您要使用的提供者特定图像。
    ...
    您在图像目录中指定的“操作系统”将显示在选择中，“描述”的内容将以绿色显示。
...
2. 为集群准备CLI JSON模板时，将“ImageCatalog”参数设置为您要使用的图像目录，并将“ImageId”参数设置为您希望的目录中图像的uuid。 使用。 例如：
